...
namespace AdventOfCode.Year2023.Day02
{
    public readonly struct Game
    {
        ...
        public static Game FromText(string text)
        {
            ...
            return new Game()
            {
                Id = Convert.ToInt32(parts[0].Split(" ").Last()),
                Sets = parts[1].Split(";").Select(x => CubeSet.FromText(x)).ToArray()
            };
        }
    }

    public readonly struct CubeSet
    {
        ...
        public static CubeSet FromText(string text)
        {
            string[] parts = text.Split(",").Select(x => x.Trim()).ToArray();
            return new CubeSet()
            {
                Red = Convert.ToInt32((parts.FirstOrDefault(x => x.Contains("red")) ?? "0").Split(" ").First()),
                Green = Convert.ToInt32((parts.FirstOrDefault(x => x.Contains("green")) ?? "0").Split(" ").First()),
                Blue = Convert.ToInt32((parts.FirstOrDefault(x => x.Contains("blue")) ?? "0").Split(" ").First()),
            };
        }
    }
    ...
    */
    ...
    {
        const int SOLUTION = 2285;

        [Fact]
        public static void Solution1()
        {
            Game[] games = File.ReadAllLines(Path.Combine("data", "day02.txt")).Select(x => Game.FromText(x)).ToArray();
            (int red, int green, int blue) allowed = (12, 13, 14);

            Game[] validGames = games.Where(game => 
                game.Sets.Select(x => x.Red).Max() <= allowed.red &&
                game.Sets.Select(set => set.Green).Max() <= allowed.green &&
                game.Sets.Select(set => set.Blue).Max() <= allowed.blue
            ).ToArray();
            int gameIdSum = validGames.Sum(game => game.Id);

            Assert.Equal(SOLUTION, gameIdSum);
        }
    }
    ...
    */
    public class Problem2
    {
        ...
        public static void Solution1()
        {
            ...
            CubeSet[] minimalBags = games.Select(game => new CubeSet()
            {
                Red = game.Sets.Select(x => x.Red).Max(),
                Green = game.Sets.Select(set => set.Green).Max(),
                Blue = game.Sets.Select(set => set.Blue).Max()
            }).ToArray();  
            int cubePowerSum = minimalBags.Sum(bag => bag.Red * bag.Green * bag.Blue);
        
            Assert.Equal(SOLUTION, cubePowerSum);
        }
    }
}
