...
namespace AdventOfCode.Year2023.Day04
{
    public struct Card
    {
        ...
        public readonly int[] WinningNumbers { get; init; }

        public readonly int[] CardNumbers { get; init; }

        public readonly int MatchCount => WinningNumbers.Intersect(CardNumbers).Count();

        public static Card FromText(string text)
        {
            string[] parts = text.Split(":");
            int cardNumber = Convert.ToInt32(parts[0].Split(" ").Last());
            parts = parts[1].Split("|").Select(x => x.Trim()).ToArray();
    ...
    */
    ...
    {
        const int SOLUTION = 25231;

        [Fact]
        public static void Solution1()
        {
            static int ScoreCard(Card card)
            {
                return card.MatchCount == 1
                    ? 1
                    : (int)Math.Pow(2, card.MatchCount - 1);
            }

            string[] lines = File.ReadAllLines(Path.Combine("data", "day04.txt"));

            Card[] cards = lines.Select(x => Card.FromText(x)).ToArray();
            int totalScore = cards.Sum(x => ScoreCard(x));

            Assert.Equal(SOLUTION, totalScore);
        }
    }
    ...
    */
    public class Problem2
    {
        ...
        public static void Solution1()
        {
            ...
            int[] copiesOfEachCard = Enumerable.Repeat(1, cards.Length).ToArray();
            for (int iCard = 0; iCard < cards.Length; iCard++)
            {
                if (cards[iCard].MatchCount > 0)
                {
                    for (int iCardToCopy = iCard + 1; iCardToCopy <= iCard + cards[iCard].MatchCount; iCardToCopy++)
                        copiesOfEachCard[iCardToCopy] += copiesOfEachCard[iCard];
                }
            }
            int totalCards = copiesOfEachCard.Sum();

            Assert.Equal(SOLUTION, totalCards);
        }
    }
}
